...
namespace Puces_R.Controles
{
    public partial class BoitePanier : System.Web.UI.UserControl
    {
        ...
        {
            SqlCommand commandePanier = new SqlCommand("SELECT TOP(1) SUM(A.NbItems * P.PrixDemande) AS SousTotal FROM PPArticlesEnPanier AS A INNER JOIN PPProduits AS P ON A.NoProduit = P.NoProduit WHERE A.NoClient = " + NoClient + " AND A.NoVendeur = " + NoVendeur + " GROUP BY A.NoVendeur", myConnexion);
